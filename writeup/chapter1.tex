...
orbits are examples of attractors. An \textbf{attractor} has the following
...
the rate of change of position on the orbit is overall greater than one (the logarithm becomes positive), then it is a
...
|\hat{\lambda}| =  \prod_{j = 1}^{p}|f'(u_j)| =\left\{
   ...
   \right.
\end{displaymath} 
As in the fixed point scenario, $|\hat{\lambda}|<1$ occurs for a
stable orbit, the $\mathcal{O}(\eta_n^2)$ terms of the Taylor
...
stability of the orbit depends on the derivative at any given point on the
...
\item period-doubling\footnote{Also known as a flip bifurcation.}:
  When a parameter increases past a certain point, a periodic orbit of
  order $2p$ appears near an orbit of order $p$. Chaos may appear if
  an infinite sequence of period-doubling bifurcations occur. 
\end{enumerate}
