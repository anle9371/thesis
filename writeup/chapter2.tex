...
\caption[The function $R(x)$]{The function $R:[0,1]\to [0,4]$ where
  $\sigma=0.0386, L=0.1, r=3.5, N=100$}\label{fig:R}
	...
	\end{center}
\end{figure}
% To clarify, the variable $\xi(x)$ is no longer normal under these
% restrictions, as it neither passes the conditions from the Lyapunov
% condition nor the Lindeberg condition~\cite{billingsley}. The Lyapunov and Lindeberg
% conditions are weaker versions of the Central Limit Theorem that
% describe how sums of nonidentical random variables may tend to the
% standard normal distribution as long as the limit of the sums of the moments of the random
% variables limit to zero. 
